...
module.exports = function generatePresetsData (dir, opts, cb) {
  ...
  presetSchema.additionalProperties = presetSchema.additionalProperties || (opts && opts.additionalProperties)
  glob('**/*.json', {cwd: dir}, function (err, files) {
    if (err) return cb(err)
    if (!files.length) console.warn('Warning: no preset json files found in %s/', dir)
    var presets = {}
